...
def train(dataloader, model, loss_fn, optimizer):
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        if (batch % len(X) == 0) or (batch + 1 == num_batches):
            ...
            print(
                f"\t[{batch+1:>3d}/{num_batches:3d} batches] "
                + f"Loss: {loss.item():>5.4f} - Accuracy: {current_acc:>5.4f}"
            )
...
for t in range(epochs):
    ...
    print(
        f"\tAvg validation loss: {avg_val_loss:>5.4f}, "
        + f"Avg validation accuracy: {avg_val_acc:>5.4f}"
    )
    ...
    print(
        ...
        + f"\tAvg loss: {avg_train_loss:>5.4f}, Avg accuracy: {avg_train_acc:>5.4f} "
...
def save_learning_curve_of_loss(
    train_loss_list: list, val_loss_list: list, date_now: str
) -> None:
    """
...
def save_learning_curve_of_acc(
    train_acc_list: list, val_acc_list: list, date_now: str
) -> None:
    """
