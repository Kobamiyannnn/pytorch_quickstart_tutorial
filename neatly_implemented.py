    ...
    for iteration, (X, y) in enumerate(dataloader):
        ...
        if (iteration % batch_size == 0) or (iteration + 1 == iterations):
            ...
            print(
                ...
                + f"- Accuracy: {acc_in_this_iteration:>5.4f}"
            )
...
class SaveLearningProgress:
    ...
    def save_lc(
        self, train_list: list, val_list: list, lc_type: Literal["Accuracy", "Loss"]
    ):
        try:
        ...
        ax.set_xticklabels(
            np.concatenate(
                [np.array([1]), np.arange(step, epochs + 1, step)], dtype="unicode"
            )
        )
        ...
        plt.savefig(
            f"{self.dir_path}/LC_{lc_type}_{self.dir_path.replace('./results/', '')}.png"
        )
        plt.close()
    ...
    def save_csv(
        ...
        list_1: list,
        list_2: list,
        data_type: Literal["learn_acc", "learn_loss", "test"],
    ):
        try:
        ...
        match data_type:
            case "learn_acc":
                df = pd.DataFrame({"train_acc": list_1, "val_acc": list_2}, index=index)
                df.to_csv(
            ...
            case "learn_loss":
                df = pd.DataFrame({"train_loss": list_1, "val_loss": list_2}, index=index)
                df.to_csv(
            ...
            case "test":
                index = ["test"]
                df = pd.DataFrame({"test_acc": list_1, "test_loss": list_2}, index=index)
                df.to_csv(
...
class EarlyStopping:
    def __init__(self, dir_path: str, model_name: str, patience: int = 5):
        ...
        # モデル保存パス
        self.path = f"{dir_path}/{dir_path.replace('./results/', '')}_{model_name}.pth"
...
if __name__ == "__main__":
    ...
    for t in range(epochs):
        ...
        print(
            f"\tAvg loss: {train_loss:>5.4f}, Avg acc: {train_acc:>5.4f} "
            + f"(Duration: {elapsed_per_epoch:.2f}s)\n",
            flush=True,
        )
